...
LRUHashTable* initLRUHashTable(int hash_table_size)
{
    ...
    if (!hash_table->hashtable) {
        perror("Failed to allocate memory for hash_table attribute "
               "->hashtable");
        free(hash_table);
        return NULL;
    }
...
int hash_table_put(LRUHashTable* hash_table, int value, Node** wrapped_node)
{
    ...
    if (*wrapped_node) {
        printf("[hash_table_put] Node already allocated at %p, no insertion "
               "performed\n",
                (void*) (*wrapped_node));
    ...
    *wrapped_node = node; /* memory delegation */
    printf("[hash_table_put] Created node at %p with value %d, delegating "
           "ownership\n",
            (void*) node, value);
    ...
    if (!last_node) {
        ...
        printf("[hash_table_put] Appended node to end of bucket %d after node "
               "%p\n",
...
Node* hash_table_get_last_bucket_node(
        const LRUHashTable* hash_table, const int idx)
{
    ...
    if (idx < 0 || idx >= hash_table->hash_table_size) {
        printf("[hash_table_get_last_bucket_node] ERROR: idx %d out of bounds "
               "(size=%d)\n",
                idx, hash_table->hash_table_size);
        return NULL;
    }
